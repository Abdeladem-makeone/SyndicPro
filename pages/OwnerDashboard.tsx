    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="bg-slate-900 rounded-[2.5rem] p-8 sm:p-12 text-white shadow-2xl flex flex-col md:flex-row justify-between items-center gap-8 relative overflow-hidden group">
         ...
         <div className="relative z-10 text-center md:text-left">
            <p className="text-[10px] font-black text-teal-400 uppercase tracking-[0.3em] mb-3">{t.balanceTitle}</p>
            ...
            <p className="text-xs mt-4 text-slate-400 font-medium max-w-xs">{t.balanceDesc}</p>
         </div>
         ...
         <div className="relative z-10 bg-white/5 p-6 rounded-3xl border border-white/10 backdrop-blur-md max-w-sm text-center md:text-left">
            <div className="flex items-center justify-center md:justify-start gap-3 mb-2">
               ...
               <p className="text-[10px] font-black uppercase tracking-widest text-teal-400">{t.transparency}</p>
            </div>
            <p className="text-[11px] text-slate-300 font-medium">{t.balanceDesc}</p>
         </div>
      </div>

      {/* Tabs Simplifiés */}
      <div className="flex gap-2">
         <button onClick={() => setActiveSubTab('finance')} className={`flex-1 sm:flex-none px-8 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all ${activeSubTab === 'finance' ? 'bg-teal-700 text-white shadow-lg' : 'bg-white text-slate-500 border border-slate-200'}`}>
           {t.paymentsTitle}
         </button>
         <button onClick={() => setActiveSubTab('notifications')} className={`flex-1 sm:flex-none px-8 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all ${activeSubTab === 'notifications' ? 'bg-teal-700 text-white shadow-lg' : 'bg-white text-slate-500 border border-slate-200'}`}>
           {t.archiveTitle}
         </button>
      </div>
        ...
        <div className="space-y-8">
           <div className="bg-white rounded-[2.5rem] p-8 sm:p-10 border border-slate-100 shadow-sm">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-6">
                 <div>
                    <h3 className="text-xl font-black text-slate-800 tracking-tight uppercase">{t.paymentsTitle} {currentYear}</h3>
                    <p className="text-xs text-slate-400 font-bold uppercase mt-1">{t.paymentsDesc}</p>
                 </div>
                 <div className="bg-teal-50 px-8 py-6 rounded-3xl border border-teal-100 text-center shadow-inner">
                    <p className="text-[10px] font-black text-teal-600 uppercase tracking-widest mb-1">{t.totalYear}</p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                 {t.months.map((month, idx) => {
                    ...
                    return (
                       <div key={month} className={`p-5 rounded-3xl border transition-all ${
                          isPaid ? 'bg-teal-50 border-teal-200 shadow-sm' : 
                          isFuture ? 'bg-slate-50 border-slate-100 opacity-40' : 'bg-rose-50 border-rose-200 shadow-inner'
                       }`}>
                          <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest text-center mb-3">{month}</p>
                          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-sm mx-auto ${
                             isPaid ? 'bg-teal-600 text-white' : 
                          ...
                          <p className={`text-[10px] font-black uppercase text-center mt-3 ${isPaid ? 'text-teal-700' : isFuture ? 'text-slate-400' : 'text-rose-600'}`}>
           ...
           <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
              <div className="p-8 border-b bg-slate-50/50 flex justify-between items-center">
                 <h3 className="text-xs font-black text-slate-800 uppercase tracking-widest">{t.registerTitle}</h3>
              </div>
              <div className="overflow-x-auto no-scrollbar">
                 <table className="w-full text-left">
                    <thead>
                       <tr className="bg-slate-50/50 border-b">
                          <th className={`px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest ${isAr ? 'text-right' : 'text-left'}`}>{t.dateLabel}</th>
                          <th className={`px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest ${isAr ? 'text-right' : 'text-left'}`}>{t.descLabel}</th>
                          <th className={`px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest ${isAr ? 'text-left' : 'text-right'}`}>{t.amountLabel}</th>
                       </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                       {expenses.filter(e => !e.excludedFromReports).slice().reverse().slice(0, 10).map(e => (
                          <tr key={e.id} className="hover:bg-slate-50/50 transition-colors">
                             <td className="px-8 py-6 text-xs text-slate-500 font-medium whitespace-nowrap">{new Date(e.date).toLocaleDateString(language === 'ar' ? 'ar-MA' : 'fr-FR')}</td>
                             <td className="px-8 py-6">
                                <p className="font-bold text-slate-800 text-sm leading-snug">{e.description}</p>
                                <p className="text-[9px] font-black text-teal-600 uppercase mt-1">{e.category}</p>
                             </td>
                             <td className={`px-8 py-6 font-black text-slate-800 text-sm whitespace-nowrap ${isAr ? 'text-left' : 'text-right'}`}>-{e.amount.toLocaleString()} <span className="text-[10px] opacity-40">MAD</span></td>
                          </tr>
                       ))}
                    </tbody>
                 </table>
      ...
      {/* Corrected typo: changed 'activeTab' to 'activeSubTab' */}
           ...
           {myNotifications.length > 0 ? myNotifications.slice().reverse().map(note => (
              <div key={note.id} className="p-6 bg-white rounded-[2rem] border border-slate-200 flex items-center gap-6 shadow-sm">
                 ...
                 <div className="flex-1">
                    <p className="text-sm font-black text-slate-800">{note.type === 'simple' ? 'Rappel standard' : 'Détail impayés'}</p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">{new Date(note.date).toLocaleString(language === 'ar' ? 'ar-MA' : 'fr-FR')}</p>
                 </div>
                 <span className="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-lg text-[10px] font-black uppercase tracking-widest">OK</span>
              </div>
           )) : (
              <div className="py-20 text-center bg-white rounded-[2rem] border border-slate-100">
                 <p className="text-sm font-black text-slate-400 uppercase tracking-widest">{t.noNotif}</p>
              </div>
           )}
        </div>
      )}
